# Training Plan: 2026-06

Growth-biased hybrid block: rebuild strength, gain size, maintain aerobic fitness, and reintroduce controlled explosiveness.

## 2026-06-01 - Maldini - Upper Strength + Calisthenics Pull

Category: gym
Run volume: 0 km
Macros: 3350 kcal, 165 g protein, 455 g carbs, 95 g fat

Adjustments:
- Shortened for schedule constraints.

Plan:
- Warm-up: 5-7 min easy bike, band pull-aparts, scap push-ups, and 2 bench ramp sets.
- Progression: use the minimum effective dose today; complete the first 3 main lifts and add accessories only if energy is good.
- Bench press: top set 1 x 4-6, then 3 back-off sets x 5-7.
- Pull-ups: 4 x 4-8 paired with chest-supported row 4 x 8-12.
- Incline dumbbell press: 3 x 8-12 paired with dips or push-ups 2-3 x clean reps.
- Core finisher: farmer carries 4 x 30-40 m plus dead bug 2 x 8 per side.

## 2026-06-02 - Baggio - Lower Strength + Knee Capacity

Category: gym
Run volume: 0 km
Macros: 3500 kcal, 165 g protein, 505 g carbs, 100 g fat

Adjustments:
- Shortened for schedule constraints.

Plan:
- Warm-up: 7-8 min bike, ankle rocks, bodyweight squats, and tibialis raises.
- Progression: use the minimum effective dose today; complete the first 3 main lifts and add accessories only if energy is good.
- Hack squat or goblet squat: top set 1 x 6-8, then 3 back-off sets x 8-10.
- Romanian deadlift: 4 x 6-8 with controlled eccentrics.
- Split squat or step-up: 3 x 8-10 per leg paired with leg curl 3 x 10-15.
- Core finisher: calf raises plus tibialis raises 4 x 12-20 each, then side plank 2 x 30 sec.

## 2026-06-03 - Pirlo - Recovery + Easy Zone 2

Category: easy_run
Run volume: 5 km
Macros: 3300 kcal, 165 g protein, 445 g carbs, 95 g fat

Plan:
- Easy run: 5 km at conversational effort; cadence light and relaxed.
- Mobility: 8-10 min hips, quads, calves, and ankles.
- Optional: 20-30 min walk later if knees feel normal.
- Fueling: fruit or rye bread plus caffeine is enough before a short morning run.

## 2026-06-04 - Buffon - Recovery / Maintenance Day

Category: maintenance
Run volume: 0 km
Macros: 3200 kcal, 165 g protein, 400 g carbs, 105 g fat

Adjustments:
- Adjusted for high-risk schedule constraints.

Plan:
- Keep training conservative because the private calendar review found a high-risk schedule constraint.
- Main work: 30-60 min walk or easy bike, then 10-12 min mobility.
- Optional: light pump circuit only if sleep, hydration, and joints feel normal.
- Recovery: protein target stays fixed; prioritize fluids, sodium, and carbohydrate-dense meals.

## 2026-06-05 - Del Piero - Recovery Day After High-Risk Schedule

Category: recovery
Run volume: 0 km
Macros: 3150 kcal, 165 g protein, 385 g carbs, 105 g fat

Adjustments:
- Adjusted after high-risk schedule constraints.

Plan:
- No heavy lower body or hard running today.
- Recovery: 30-60 min walk, 10 min mobility, hydration, sodium, and easy meals with carbs.
- Optional: light upper accessories only if energy is clearly good.

## 2026-06-06 - Totti - Recovery Day After High-Risk Schedule

Category: recovery
Run volume: 0 km
Macros: 3150 kcal, 165 g protein, 385 g carbs, 105 g fat

Adjustments:
- Sprint moved away from a high-risk day.

Plan:
- No sprinting today.
- Recovery: 30-60 min walk, 10 min mobility, hydration, sodium, and easy meals with carbs.
- If energy is good later, add light upper accessories only: rows 2 x 12, lateral raises 2 x 20, curls 2 x 15.
- Macro target stays the same; prioritize fluids and carbohydrates early.

## 2026-06-07 - Cannavaro - Recovery Day After High-Risk Schedule

Category: recovery
Run volume: 3 km
Macros: 3150 kcal, 165 g protein, 385 g carbs, 105 g fat

Adjustments:
- Adjusted after high-risk schedule constraints.

Plan:
- No heavy lower body or hard running today.
- Recovery: 30-60 min walk, 10 min mobility, hydration, sodium, and easy meals with carbs.
- Optional: light upper accessories only if energy is clearly good.

## 2026-06-08 - Nesta - Full-Body Athletic Strength

Category: gym
Run volume: 0 km
Macros: 3500 kcal, 165 g protein, 505 g carbs, 100 g fat

Adjustments:
- Shortened for schedule constraints.

Plan:
- Warm-up: 7 min easy cardio, squat-to-stand, band rows, push-ups, and hinge ramp sets.
- Progression: use the minimum effective dose today; complete the first 3 main lifts and add accessories only if energy is good.
- Incline press or bench press: top set 1 x 6-8, then 3-4 back-off sets x 8-10.
- Trap-bar deadlift, Romanian deadlift, or heavy hinge: 3-4 x 5-6.
- Pull-ups or chest-supported rows: 4 x 6-10 paired with goblet squat 3 x 8-12.
- Core finisher: farmer carries 3 x 40 m plus dead bug 2 x 8 per side.

## 2026-06-09 - Baresi - Easy Run + Relaxed Strides

Category: easy_run
Run volume: 6 km
Macros: 3300 kcal, 165 g protein, 445 g carbs, 95 g fat

Plan:
- Easy run: 6 km conversational.
- Strides: 4 x 15 sec relaxed on flat ground only if knees feel normal.
- Mobility: 8 min calves, quads, glutes, and ankles.
- Fueling: small carb snack before morning running if sleep was poor.

## 2026-06-10 - Zoff - Lower Unilateral + Posterior Chain

Category: gym
Run volume: 0 km
Macros: 3500 kcal, 165 g protein, 505 g carbs, 100 g fat

Adjustments:
- Shortened for schedule constraints.

Plan:
- Warm-up: 7-8 min bike, split squat isometric holds, ankle rocks, and hinge ramp sets.
- Progression: use the minimum effective dose today; complete the first 3 main lifts and add accessories only if energy is good.
- Bulgarian split squat or step-up: 3-4 x 6-10 per leg.
- Romanian deadlift: 3 x 8 paired with leg curl 3 x 10-15.
- Walking lunges: 2-3 x 12 steps per leg.
- Core finisher: calf raises plus Copenhagen plank 2-3 sets each.

## 2026-06-11 - Rossi - Calisthenics Density + Zone 2

Category: gym
Run volume: 4 km
Macros: 3500 kcal, 165 g protein, 505 g carbs, 100 g fat

Adjustments:
- Adjusted for schedule constraints.

Plan:
- Warm-up: 5 min easy cardio, shoulder prep, hip mobility, and crawling patterns.
- Progression: double progression by adding clean reps before adding time or load. Reduce volume by 25-40% before adding intensity.
- Density block: 20 min alternating pull-ups, push-ups, inverted rows, and hanging knee raises.
- Easy run: 4 km conversational immediately after or in the morning.
- Carry finisher: suitcase carry 3 x 30 m per side.
- Optional: lateral raises or curls 2 x 15-20 if time remains.

## 2026-06-12 - Rivera - Recovery + Mobility Capacity

Category: recovery
Run volume: 0 km
Macros: 3150 kcal, 165 g protein, 385 g carbs, 105 g fat

Plan:
- Recovery: 30-60 min walk or easy bike.
- Mobility: hips, ankles, thoracic spine, calves, and quads for 12-15 min.
- Knee capacity: tibialis raises 2 x 20, slow calf raises 2 x 15, wall sit 2 x 30-45 sec.
- Optional: light crawling or plank variations for 5 min.
- Fueling: keep protein at target and do not under-eat on the recovery day.

## 2026-06-13 - Scirea - Plyometrics + Functional Power

Category: sprint
Run volume: 3 km
Macros: 3550 kcal, 165 g protein, 520 g carbs, 100 g fat

Adjustments:
- Adjusted for schedule constraints.

Plan:
- Warm-up jog: 10 min easy plus skips, ankling, hip openers, and 3 relaxed strides.
- Progression: add power volume only when jumps stay crisp and knees stay quiet the next day. Reduce volume by 25-40% before adding intensity.
- Plyometrics: pogos 3 x 20 sec, low broad jumps 4 x 2, and med-ball slams 4 x 5 if available.
- Hill sprint technique: 4-6 x 8-10 sec at fast but controlled effort.
- Functional work: sled push if easy to set up or farmer carries 4 x 30 m.
- Core: crawling 3 x 20 m or dead bug 3 x 8 per side.

## 2026-06-14 - Zambrotta - Aerobic Base + Reset

Category: long_run
Run volume: 9 km
Macros: 3550 kcal, 165 g protein, 520 g carbs, 100 g fat

Adjustments:
- Adjusted for schedule constraints.

Plan:
- Easy run: 9 km conversational; shorten to 6 km if sleep or knees are poor.
- Optional: 4 x 20 sec relaxed strides only if the week felt easy.
- Recovery: long mobility reset for calves, quads, hips, and feet. Reduce volume by 25-40% before adding intensity.
- Fueling: carbs before and after; keep this aerobic, not competitive.

## 2026-06-15 - Gattuso - Upper Strength + Calisthenics Pull

Category: gym
Run volume: 0 km
Macros: 3350 kcal, 165 g protein, 455 g carbs, 95 g fat

Adjustments:
- Adjusted for schedule constraints.

Plan:
- Warm-up: 5-7 min easy bike, band pull-aparts, scap push-ups, and 2 bench ramp sets.
- Progression: double progression; add load only after all sets reach the top of the range with 1-2 reps in reserve. Reduce volume by 25-40% before adding intensity.
- Bench press: top set 1 x 4-6, then 3 back-off sets x 5-7.
- Pull-ups: 4 x 4-8 paired with chest-supported row 4 x 8-12.
- Incline dumbbell press: 3 x 8-12 paired with dips or push-ups 2-3 x clean reps.
- Core finisher: farmer carries 4 x 30-40 m plus dead bug 2 x 8 per side.

## 2026-06-16 - Inzaghi - Lower Strength + Knee Capacity

Category: gym
Run volume: 0 km
Macros: 3500 kcal, 165 g protein, 505 g carbs, 100 g fat

Adjustments:
- Adjusted for schedule constraints.

Plan:
- Warm-up: 7-8 min bike, ankle rocks, bodyweight squats, and tibialis raises.
- Progression: double progression; add load only when knee tracking stays clean and discomfort stays 0-2/10. Reduce volume by 25-40% before adding intensity.
- Hack squat or goblet squat: top set 1 x 6-8, then 3 back-off sets x 8-10.
- Romanian deadlift: 4 x 6-8 with controlled eccentrics.
- Split squat or step-up: 3 x 8-10 per leg paired with leg curl 3 x 10-15.
- Core finisher: calf raises plus tibialis raises 4 x 12-20 each, then side plank 2 x 30 sec.

## 2026-06-17 - Vieri - Recovery + Easy Zone 2

Category: easy_run
Run volume: 5 km
Macros: 3300 kcal, 165 g protein, 445 g carbs, 95 g fat

Plan:
- Easy run: 5 km at conversational effort; cadence light and relaxed.
- Mobility: 8-10 min hips, quads, calves, and ankles.
- Optional: 20-30 min walk later if knees feel normal.
- Fueling: fruit or rye bread plus caffeine is enough before a short morning run.

## 2026-06-18 - Mazzola - Upper Calisthenics + Hypertrophy

Category: gym
Run volume: 0 km
Macros: 3350 kcal, 165 g protein, 455 g carbs, 95 g fat

Adjustments:
- Adjusted for schedule constraints.

Plan:
- Warm-up: 5 min easy cardio, shoulder circles, light rows, and scap pull-ups.
- Progression: double progression; beat total quality reps before adding load or a set. Reduce volume by 25-40% before adding intensity.
- Pull-ups: 5 x 4-8 or 20-30 total quality reps.
- Dips or push-ups: 3 x 8-15.
- One-arm dumbbell row or inverted row: 3 x 8-12 paired with incline dumbbell press 2-3 x 8-12.
- Core finisher: lateral raises plus hanging leg raises 2-3 sets each.

## 2026-06-19 - Tardelli - Posterior Chain + Functional Carries

Category: gym
Run volume: 0 km
Macros: 3500 kcal, 165 g protein, 505 g carbs, 100 g fat

Adjustments:
- Adjusted for schedule constraints.

Plan:
- Warm-up: 7-8 min bike, glute bridges, ankle mobility, and light hinge ramp sets.
- Progression: double progression; add load only when hinge speed and hamstring control stay crisp. Reduce volume by 25-40% before adding intensity.
- Romanian deadlift: top set 1 x 5-7, then 3 back-off sets x 6-8.
- Hip thrust or glute bridge: 3 x 8-12 paired with front-foot elevated split squat 2-3 x 8-10.
- Farmer carries: 3 x 30-40 m.
- Core finisher: Pallof press or side plank 3 sets.

## 2026-06-20 - Chiellini - Recovery / Maintenance Day

Category: recovery
Run volume: 3 km
Macros: 3150 kcal, 165 g protein, 385 g carbs, 105 g fat

Adjustments:
- Adjusted for high-risk schedule constraints.

Plan:
- Keep training conservative because the private calendar review found a high-risk schedule constraint.
- Main work: 30-60 min walk or easy bike, then 10-12 min mobility.
- Optional: light pump circuit only if sleep, hydration, and joints feel normal.
- Recovery: protein target stays fixed; prioritize fluids, sodium, and carbohydrate-dense meals.

## 2026-06-21 - Bonucci - Recovery Day After High-Risk Schedule

Category: recovery
Run volume: 3 km
Macros: 3150 kcal, 165 g protein, 385 g carbs, 105 g fat

Adjustments:
- Adjusted after high-risk schedule constraints.

Plan:
- No heavy lower body or hard running today.
- Recovery: 30-60 min walk, 10 min mobility, hydration, sodium, and easy meals with carbs.
- Optional: light upper accessories only if energy is clearly good.

## 2026-06-22 - Riva - Full-Body Athletic Strength

Category: gym
Run volume: 0 km
Macros: 3500 kcal, 165 g protein, 505 g carbs, 100 g fat

Adjustments:
- Shortened for schedule constraints.

Plan:
- Warm-up: 7 min easy cardio, squat-to-stand, band rows, push-ups, and hinge ramp sets.
- Progression: use the minimum effective dose today; complete the first 3 main lifts and add accessories only if energy is good.
- Incline press or bench press: top set 1 x 6-8, then 3-4 back-off sets x 8-10.
- Trap-bar deadlift, Romanian deadlift, or heavy hinge: 3-4 x 5-6.
- Pull-ups or chest-supported rows: 4 x 6-10 paired with goblet squat 3 x 8-12.
- Core finisher: farmer carries 3 x 40 m plus dead bug 2 x 8 per side.

## 2026-06-23 - Facchetti - Easy Run + Relaxed Strides

Category: easy_run
Run volume: 6 km
Macros: 3300 kcal, 165 g protein, 445 g carbs, 95 g fat

Plan:
- Easy run: 6 km conversational.
- Strides: 4 x 15 sec relaxed on flat ground only if knees feel normal.
- Mobility: 8 min calves, quads, glutes, and ankles.
- Fueling: small carb snack before morning running if sleep was poor.

## 2026-06-24 - Antognoni - Lower Unilateral + Posterior Chain

Category: gym
Run volume: 0 km
Macros: 3500 kcal, 165 g protein, 505 g carbs, 100 g fat

Adjustments:
- Shortened for schedule constraints.

Plan:
- Warm-up: 7-8 min bike, split squat isometric holds, ankle rocks, and hinge ramp sets.
- Progression: use the minimum effective dose today; complete the first 3 main lifts and add accessories only if energy is good.
- Bulgarian split squat or step-up: 3-4 x 6-10 per leg.
- Romanian deadlift: 3 x 8 paired with leg curl 3 x 10-15.
- Walking lunges: 2-3 x 12 steps per leg.
- Core finisher: calf raises plus Copenhagen plank 2-3 sets each.

## 2026-06-25 - Cabrini - Calisthenics Density + Zone 2

Category: gym
Run volume: 4 km
Macros: 3500 kcal, 165 g protein, 505 g carbs, 100 g fat

Adjustments:
- Shortened for schedule constraints.

Plan:
- Warm-up: 5 min easy cardio, shoulder prep, hip mobility, and crawling patterns.
- Progression: use the minimum effective dose today; complete the first 3 main lifts and add accessories only if energy is good.
- Density block: 20 min alternating pull-ups, push-ups, inverted rows, and hanging knee raises.
- Easy run: 4 km conversational immediately after or in the morning.
- Carry finisher: suitcase carry 3 x 30 m per side.
- Optional: lateral raises or curls 2 x 15-20 if time remains.

## 2026-06-26 - Bergomi - Recovery + Mobility Capacity

Category: recovery
Run volume: 0 km
Macros: 3150 kcal, 165 g protein, 385 g carbs, 105 g fat

Plan:
- Recovery: 30-60 min walk or easy bike.
- Mobility: hips, ankles, thoracic spine, calves, and quads for 12-15 min.
- Knee capacity: tibialis raises 2 x 20, slow calf raises 2 x 15, wall sit 2 x 30-45 sec.
- Optional: light crawling or plank variations for 5 min.
- Fueling: keep protein at target and do not under-eat on the recovery day.

## 2026-06-27 - Donadoni - Recovery / Maintenance Day

Category: recovery
Run volume: 3 km
Macros: 3150 kcal, 165 g protein, 385 g carbs, 105 g fat

Adjustments:
- Adjusted for high-risk schedule constraints.

Plan:
- Keep training conservative because the private calendar review found a high-risk schedule constraint.
- Main work: 30-60 min walk or easy bike, then 10-12 min mobility.
- Optional: light pump circuit only if sleep, hydration, and joints feel normal.
- Recovery: protein target stays fixed; prioritize fluids, sodium, and carbohydrate-dense meals.

## 2026-06-28 - Mancini - Recovery Day After High-Risk Schedule

Category: recovery
Run volume: 3 km
Macros: 3150 kcal, 165 g protein, 385 g carbs, 105 g fat

Adjustments:
- Adjusted after high-risk schedule constraints.

Plan:
- No heavy lower body or hard running today.
- Recovery: 30-60 min walk, 10 min mobility, hydration, sodium, and easy meals with carbs.
- Optional: light upper accessories only if energy is clearly good.

## 2026-06-29 - Vialli - Recovery / Maintenance Day

Category: maintenance
Run volume: 0 km
Macros: 3200 kcal, 165 g protein, 400 g carbs, 105 g fat

Adjustments:
- Adjusted for high-risk schedule constraints.

Plan:
- Keep training conservative because the private calendar review found a high-risk schedule constraint.
- Main work: 30-60 min walk or easy bike, then 10-12 min mobility.
- Optional: light pump circuit only if sleep, hydration, and joints feel normal.
- Recovery: protein target stays fixed; prioritize fluids, sodium, and carbohydrate-dense meals.

## 2026-06-30 - Albertini - Recovery Day After High-Risk Schedule

Category: recovery
Run volume: 0 km
Macros: 3150 kcal, 165 g protein, 385 g carbs, 105 g fat

Adjustments:
- Adjusted after high-risk schedule constraints.

Plan:
- No heavy lower body or hard running today.
- Recovery: 30-60 min walk, 10 min mobility, hydration, sodium, and easy meals with carbs.
- Optional: light upper accessories only if energy is clearly good.
